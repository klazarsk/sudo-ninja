...
#
for cmdTest in sed grep visudo tr visudo;
do
...
# if we are, don't run the spinner!
  ...
  then
    ...
    echo -en "\x1b[2K\r\e[1;91m>\e[1;93m ${gfxSpin} ${ctag} ${otagBold}${strStep}${ctag}\r";
    case "${gfxSpin}" in
      "/" ) gfxSpin="-";
        ;;
      "-" ) gfxSpin="\\";
...
then
  ...
  ${cmdDbgRead} -n 1 -s -r -p "Line ${LINENO} : Press any key to continue..." ;
    echo -e "\n\n Checking ${optOutputFile} syntax and integrity with visudo:"  | ${cmdTee} "${fileLog}";
  visudo -c -f "${optOutputFile}" ;
fi;
